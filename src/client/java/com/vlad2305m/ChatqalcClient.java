...
import static com.vlad2305m.MathEngine.reformatMinecraftAnsi;
...
public class ChatqalcClient implements ClientModInitializer {
	...
	public static boolean executeToChat(TextFieldWidget field) {
		...
		if (originalText.isBlank())
			return false;
		TriConsumer<Text, MessageSignatureData, MessageIndicator> textConsumer = MinecraftClient.getInstance().inGameHud
				.getChatHud()::addMessage;
		MathEngine.addMessage = (s) -> {
			if (s != null) {
				textConsumer.accept(Text.literal(MathEngine.reformatAnsiMinecraft(s)), null,
						new MessageIndicator(15677346, null, Text.literal("Qalculate!"), "chatqalc"));
			}
		};
	...
	public static boolean executeToInput(TextFieldWidget field) {
		...
		MathEngine.addMessage = (s) -> {
			if (!s.isBlank())
				field.setText(stripAnsi(s.strip()));
		};
		MathEngine.eval(originalText);
		MinecraftClient.getInstance().inGameHud.getChatHud().addToMessageHistory(originalText);
		return true;
	...
	public static boolean substituteWord(TextFieldWidget field) {
		...
		int i;
		for (i = originalText.lastIndexOf(" ", end - 1); i > 0
				&& originalText.charAt(i - 1) == '\\'; i = originalText.lastIndexOf(" ", i - 1))
			;
		int start = i + 1;
		...
		MathEngine.addMessage = (s) -> {
			if (s != null && !s.isBlank()) {
				...
				field.setCursor(start + s.length(), false);
			}
		};
		...
		MathEngine.addMessage = (s) -> {
			if (s != null && !s.isBlank() && !s.equals("  \033[0;36m0\033[0m = \033[0;36m0\033[0m")) {
				...
				if (s.length() > 1 && s.charAt(0) == '>')
					field.setText(stripAnsi(s.substring(1).strip()));
			}
		};
